...
Iout=np.sum(data.T[4]==5)
Itot=np.sum(data.T[4])

P=Iout/float(Itot)

for i in data:
	if(i[4]==5):
		ix=int((i[iX]-center[iY]+rout)/delta)
		iy=int((i[iX]-center[iY]+rout)/delta)
		dist[ix][iy]+=1
...
pl=axs.contourf(X*1e3,Y*1e3,dist/np.max(dist),levels=np.linspace(0,1,100),cmap="gray")
